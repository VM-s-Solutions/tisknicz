  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-brand-600">Produkty</p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-zinc-900">Moje produkty</h1>
        </div>
        <ProductActions categories={categories ?? []} />
      </div>

      {(!products || products.length === 0) ? (
        <div className="mt-10 flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-200 py-16">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-zinc-100">
            <Icon name="package" size={28} className="text-zinc-400" />
          </div>
          <p className="mt-4 text-sm font-medium text-zinc-600">Zatím nemáte žádné produkty.</p>
          <p className="mt-1 text-xs text-zinc-400">Klikněte na &quot;Přidat produkt&quot; a začněte prodávat.</p>
        </div>
      ) : (
        <div className="mt-8 space-y-4">
          {products.map((product) => {
            ...
            return (
              <div key={product.id} className="flex items-center justify-between rounded-2xl border border-zinc-100 bg-white p-5 shadow-md transition-all duration-200 hover:shadow-lg">
                <div>
