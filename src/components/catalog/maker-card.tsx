...
export function MakerCard({ maker, slug }: MakerCardProps) {
  return (
    <Link href={`/katalog/${slug}`} className="group block">
      <div className="hover-glow overflow-hidden rounded-2xl border border-zinc-800 bg-surface-card transition-all duration-300 group-hover:-translate-y-1">
        {/* Gradient header */}
        <div className="relative bg-gradient-to-br from-zinc-800 via-zinc-800 to-zinc-900 p-6">
          <div className="absolute inset-0 bg-dot-pattern opacity-5" />
          <div className="relative flex items-center gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-zinc-700/50 text-xl font-bold text-white backdrop-blur-sm">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                {maker.is_verified && (
                  <Icon name="verified" size={18} className="shrink-0 text-brand-400" />
                )}
              </div>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              {maker.rating_count > 0 ? (
                ...
                <span className="text-xs text-zinc-600">Zatím bez hodnocení</span>
              )}
            </div>
