...
/// <summary>
/// End-to-end coverage for the T-0076 customer-host MarkOrderDelivered
/// endpoint (<c>POST /api/v1/customer/orders/{orderId}/deliver</c>).
/// Real Postgres via <see cref="PostgresHarness"/> so the UoW pipeline
/// commits the Shipped → Delivered transition + outbox row atomically per
/// ADR 0014. Pins the observable invariants: state = Delivered,
/// delivery_source = 0 (Customer), and exactly one outbox row with
/// event_type <see cref="OutboxEventTypes.OrderDeliveredCustomerEmail"/>.
/// </summary>
[Collection(PostgresCollection.Name)]
public sealed class MarkOrderDeliveredIntegrationTests : IAsyncLifetime
{
    ...
    public MarkOrderDeliveredIntegrationTests(PostgresHarness harness)
    {
        _harness = harness;
    }
    ...
    [Fact]
    public async Task MarkDelivered_end_to_end_transitions_Shipped_to_Delivered_and_writes_1_outbox_row()
    {
        ...
        var response = await client.PostAsync(
            $"/api/v1/orders/{OrderId}/deliver", content: null);
        ...
        row.DeliverySource.Should().Be(OrderDeliverySource.Customer);

        var outboxRows = await db.Set<OutboxEvent>().AsNoTracking()
            .Where(e => e.AggregateId == OrderId)
            .ToListAsync();
        outboxRows.Should().HaveCount(1,
            "MarkOrderDelivered enqueues exactly one customer-notification event per T-0076");
        outboxRows[0].EventType.Should().Be(OutboxEventTypes.OrderDeliveredCustomerEmail);
    }
}
