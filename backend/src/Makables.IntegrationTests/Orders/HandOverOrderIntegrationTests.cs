...
/// <summary>
/// End-to-end coverage for the T-0073 maker-host HandOverOrder endpoint
/// (<c>POST /api/v1/orders/{orderId}/handover</c>) on a PersonalPickup
/// order. Mirror of <see cref="ShipOrderIntegrationTests"/> but pins the
/// distinguishing constraints from T-0073 locked decision A.1:
/// <list type="bullet">
///   <item><description>No carrier ref / tracking URL on the row.</description></item>
///   <item><description>EXACTLY one outbox event (the customer-shipped
///     email); no <see cref="OutboxEventTypes.ShippingGenerateLabel"/>
///     enqueued — there is no carrier-side label to fetch.</description></item>
///   <item><description>Payload <c>TrackingUrl</c> field is null
///     (template conditionally renders the tracking row).</description></item>
/// </list>
/// </summary>
[Collection(PostgresCollection.Name)]
public sealed class HandOverOrderIntegrationTests : IAsyncLifetime
{
    ...
    public HandOverOrderIntegrationTests(PostgresHarness harness)
    {
        _harness = harness;
    }
    ...
    private async Task SeedAcceptedPersonalPickupOrderAsync()
    {
        ...
        // PersonalPickup: shipping is free, no pickup point id.
        var order = Order.Create(
            ...
            productPriceAmountMinor: 50000, shippingPriceAmountMinor: 0,
            platformFeeAmountMinor: 7500, makerPayoutAmountMinor: 42500,
            totalAmountMinor: 50000, currency: Currency, vatRateBp: 2100,
    ...
    [Fact]
    public async Task HandOverOrder_PersonalPickup_end_to_end_emits_1_outbox_event_with_null_tracking_url()
    {
        await SeedAcceptedPersonalPickupOrderAsync();
        using var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Bearer", IssueMakerToken());

        var response = await client.PostAsync(
            $"/api/v1/orders/{OrderId}/handover", content: null);
        ...
        row.ShippingCarrierRef.Should().BeNull("personal pickup has no carrier");
        row.ShippingCarrierTrackingUrl.Should().BeNull("personal pickup has no tracking URL");

        var outboxRows = await db.Set<OutboxEvent>().AsNoTracking()
            .Where(e => e.AggregateId == OrderId)
            .ToListAsync();
        outboxRows.Should().HaveCount(1,
            "HandOverOrder emits exactly one customer-email event — no generate-label per T-0073");
        outboxRows[0].EventType.Should().Be(OutboxEventTypes.OrderShippedCustomerEmail);

        var payload = JsonSerializer.Deserialize<OrderShippedCustomerEmailPayload>(
            outboxRows[0].PayloadJson);
        payload.Should().NotBeNull();
        payload!.OrderId.Should().Be(OrderId);
        payload.TrackingUrl.Should().BeNull(
            "personal-pickup payload's TrackingUrl is null — template hides the tracking row");
    }
}
