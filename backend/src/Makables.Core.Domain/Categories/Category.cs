...
public sealed class Category : Auditable
{
    ...
    /// <summary>
    /// Generate a Czech-friendly URL slug. Delegates to the shared
    /// <see cref="SlugGenerator"/> (T-0043 extracted the duplicate that
    /// Maker also needed). Kept as a static on Category for call-site
    /// readability + backwards-compat with existing callers/tests.
    /// </summary>
    public static string Slugify(string value) => SlugGenerator.Slugify(value);

    private static bool IsValidSlug(string slug) =>
        slug.Length > 0 && SlugGenerator.IsValid(slug);
}
