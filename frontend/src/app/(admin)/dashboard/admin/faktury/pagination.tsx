...
interface PaginationProps {
  ...
  /** Filter params to preserve across pages (type/country/recipient/dates). */
  readonly baseParams: Readonly<Record<string, string>>;
}

/**
 * URL-driven prev/next pagination for the admin invoices list (T-0118a —
 * local copy of the admin orders pagination pointed at the `/faktury`
 * base path). `<Link>`-based; only `page > 1` is emitted (`page=1`
 * dropped, B.8); the active filters ride along.
 */
...
export function Pagination({ page, totalPages, hasNext, hasPrevious, baseParams }: PaginationProps) {
  ...
  return (
    <nav
      aria-label={t('dashboard.admin.invoices.pagination.page_of', { page, total: totalPages })}
    ...
    >
      {hasPrevious ? (
        ...
        >
          ...
          {t('dashboard.admin.invoices.pagination.previous')}
        </Link>
      ) : (
        <span
          aria-disabled="true"
      ...
      <p className="text-sm text-zinc-400" aria-live="polite">
        {t('dashboard.admin.invoices.pagination.page_of', { page, total: totalPages })}
      </p>
      ...
      {hasNext ? (
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-zinc-700 px-4 py-2.5 text-sm font-semibold text-zinc-300 transition-colors hover:border-zinc-600 hover:bg-zinc-800"
        >
          {t('dashboard.admin.invoices.pagination.next')}
