      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8">
          {result.success ? (
            ...
            <OrdersError error={result.error} retryHref={retryHref(ROUTE_PATH, paginationParams, page)} />
          )}
        </div>
      </div>
    </section>
...
function OrdersResults({ data, baseParams }: OrdersResultsProps) {
  if (data.totalCount === 0) {
    return <OrdersEmpty hasFilters={Object.keys(baseParams).length > 0} />;
  }
 ...
 */
function OrdersEmpty({ hasFilters }: { readonly hasFilters: boolean }) {
  if (hasFilters) {
    return (
...
function OrdersError({ error, retryHref }: { readonly error: ApiError; readonly retryHref: string }) {
  return (
    <Alert variant="error">
