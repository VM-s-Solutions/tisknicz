...
/**
 * URL-driven prev/next pagination for the maker payout list (T-0116 — local
 * copy of the objednavky resolution pointed at the `/vyplaty` base path).
 * `<Link>`-based so the back button works and the page stays a Server
 * Component; disabled state uses a non-link span. Only `page > 1` is emitted
 * (`page=1` dropped, patterns.md B.8). The list takes no other params.
 */
export function Pagination({ page, totalPages, hasNext, hasPrevious }: PaginationProps) {
  ...
  const hrefFor = (target: number): string =>
    target > 1 ? `/dashboard/maker/vyplaty?page=${target}` : '/dashboard/maker/vyplaty';

  return (
    <nav
      aria-label={t('dashboard.maker.payouts.pagination.page_of', { page, total: totalPages })}
    ...
    >
      ...
      <p className="text-sm text-zinc-400" aria-live="polite">
        {t('dashboard.maker.payouts.pagination.page_of', { page, total: totalPages })}
      </p>
