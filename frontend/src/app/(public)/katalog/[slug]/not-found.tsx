...
/**
 * 404 surface for the maker profile (T-0047 AC-3). Renders when the
 * page's <c>notFound()</c> fires — the helper returns an
 * <c>ApiError</c> of type <c>NotFound</c> on either an unknown slug or
 * an inactive / unconfirmed maker (the backend treats them the same).
 */
export default function MakerNotFound() {
  return (
    <section className="mx-auto flex min-h-[calc(100vh-64px)] max-w-3xl flex-col items-center justify-center px-4 py-10 text-center sm:px-6 lg:px-8">
      ...
      <p className="mt-3 text-base text-zinc-400">{t('catalog.maker.not_found.body')}</p>
      <div className="mt-8">
        <Link
          href="/katalog"
