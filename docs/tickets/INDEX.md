...
| T-0067 | Extend T-0066's `MarkOrderPaid.Handler` with outbox emission (2 events at MVP: customer order-paid email + maker order-placed email; **invoice.generate deferred to T-0068** per user decision Q2). Ships `AddOrderPaymentMethod` migration + extends `Order.MarkAsPaid` to `(clock, providerRef, paymentMethod, paidAtOverride)` so T-0066's Command fields actually get persisted. Widens `WebhookPayload` with nullable `PaidAt` to trust Comgate's authoritative timestamp. Refactors `EmailSendService.SendAsync` to a per-event-type switch branch (first non-auth email shape). Pre-bakes action URLs into outbox payloads. New `EmailTemplateType.OrderPaidCustomer` + `OrderPlacedMaker` enum values + seed-row migration (cs-CZ + en-US translations) | 4 | M | **ready** | T-0066, T-0011, T-0028 | US-customer-0010, US-maker-0006 | 0014, 0016, 0019, 0020 |
