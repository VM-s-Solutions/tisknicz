...
| T-0066 | Comgate webhook controller (`POST /api/v1/public/webhooks/comgate`) on Web.Public: three-layer security (`IComgateWebhookIpAllowlist` filter accepting bare IPs + CIDR → re-fetch via `VerifyPaymentAsync` → idempotency check via `GetByPaymentProviderRefAsync`); replaces `ParseAndVerifyWebhookAsync` stub from T-0065; ships **stub `MarkOrderPaid.Command`** doing state transition only (no outbox emission — T-0067 territory); 3 new `payment.webhook.*` BusinessErrorMessage codes + Czech i18n | 4 | M | **ready** | T-0065 | US-customer-0010 | 0002, 0005, 0016, 0020 |
| T-0067 | Extend T-0066's `MarkOrderPaid.Handler` to enqueue outbox events: customer email (`order-paid`), maker email (`new-order`), invoice.generate. Ships `AddOrderPaymentMethodAndPaidAt` migration to persist the two fields T-0066 accepts but ignores. State transition itself stays in `Order.MarkAsPaid` from T-0060 | 4 | M | draft | T-0066, T-0011 | US-customer-0010 | 0016, 0020 |
