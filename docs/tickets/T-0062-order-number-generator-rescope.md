...
- 2026-06-05 third Copilot review on PR — 1 Medium on a comment claim in `First_allocation_race_creates_row_exactly_once`. **FOLDED.** The original comment said the loser of the first-allocation race would be translated to a typed `BusinessResult.Failure(Conflict)` by `UnitOfWorkPipelineBehavior`. That's wrong — `PK_numbering_sequence` is **not** in `UniqueConstraintTranslator.Mappings`, so the loser surfaces `UniqueConstraintViolationException` verbatim (the test itself already asserts on that exception; only the comment was misleading). The behaviour is intentional and consistent with the file's own "defence-in-depth invariants stay unmapped" policy — same rationale as `ix_orders_order_number` (T-0060 M-1): a monotonic generator should never produce a colliding insert, so a 23505 here means the generator was bypassed or broke and should stay visible as the typed exception rather than be masked into a generic Conflict. Rewrote the comment to (a) describe the actual runtime contract (loser → `UniqueConstraintViolationException`; retry hits the row + increments to 0002) and (b) document the path to changing the policy if the team later wants typed conflict translation. Comment-only change; build clean; test still passes.
