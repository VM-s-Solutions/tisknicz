...
1. **QuestPDF license tier = Community (MIT, free).** Confirmed by user 2026-06-07. JVM YORE s.r.o. is a Czech s.r.o. with revenue < $1M USD AND < 10 employees AND not state-funded — meets all three Community criteria. Pin `QuestPDF.Settings.License = LicenseType.Community` at the `Infra.PdfRendering` project's startup hook (referenced from each `Web.*` host's `Program.cs` via the new `AddMakablesPdfRendering()` extension). **ADR 0025 (or next number)** documents the dual-license criteria so PM's sprint-checkpoint review catches the moment JVM YORE crosses any of the thresholds (revenue, employee count) and triggers a switch to QuestPDF Professional. The switch is one config line + a Key Vault secret for the license key — non-invasive architecturally. **Rejected alternative:** QuestPDF Pro (€699/yr) now — premature; Community is correct for current posture.
...
8. **Issuer values from `country_configurations` columns + CZ seed migration.** Schema change: add 3 columns to `country_configurations`: `issuer_name VARCHAR(200) NOT NULL`, `issuer_ico CHAR(8) NOT NULL`, `issuer_dic VARCHAR(15) NULL` (nullable per T-0068a locked decision 2 — JVM YORE is not VAT-registered). CZ seed in the same migration: `issuer_name='JVM YORE s.r.o.'`, `issuer_ico='00000000'` (placeholder per user direction — replaced pre-launch via a one-line seed-update migration), `issuer_dic=NULL`. The placeholder is NOT a PR-blocker: tests + builds pass with `'00000000'`; `Invoice.Issue` factory validates only that `IssuerIco` is exactly 8 chars (it does NOT validate via the Czech IČO mod-11 algorithm — that's handled by ARES during maker registration, NOT for the platform's own issuer field). The `country-config-ico-replace-placeholder-pre-launch` manual_step is a pre-prod-deploy task tracked in this ticket's frontmatter so it surfaces in PM sprint checkpoint reports.
...
- 2026-06-07 manual_steps clarified by PM after user follow-up: (a) QuestPDF Community license confirmed appropriate for JVM YORE's current posture — Pro revisit deferred to PM sprint-checkpoint review on revenue/headcount milestones. (b) CZ seed `issuer_ico` ships with placeholder `'00000000'` per user direction; replaced pre-production-launch via a one-line data migration. Neither item is a PR-open blocker; both tracked in this ticket's frontmatter `manual_steps`. **NOT YET DISPATCHABLE** — implementation requires T-0068a (PR #32, merged) AND T-0067 (already done) AND this grooming PR (current, awaiting merge) to all land on master.
