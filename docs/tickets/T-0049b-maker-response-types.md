...
- 2026-06-01 done. Build clean, 773 unit + 82 integration tests pass (matches T-0049a baseline). Spec verified: `productsPOST` → `CreateProductResponse`, `imagesPOST` → `UploadProductImageResponse`, `productsPUT`/`productsDELETE`/`imagesDELETE` typed as `Promise<void>`. Spec hash is tracked in `frontend/src/lib/api-client/.spec-hashes.json`'s `maker-api.v1` entry — the file is the source of truth so this log doesn't drift on subsequent regenerations (the hash inevitably changes as Copilot rounds add attributes / schema transformers). Frontend `tsc --noEmit` + `lint` clean. No dual-reviewer pass — change is mechanical (attributes + controller-level response wrap + a small one-off script appendix).
- 2026-06-01 Copilot review folded — six findings.
  - **M1 + M2 — 401 missing on the two read endpoints.** Both `GET /products` and `GET /products/{productId}` are `[Authorize]`d but their OpenAPI metadata declared only 200 + 404. Added `[ProducesResponseType(typeof(Error), StatusCodes.Status401Unauthorized)]` so the regenerated client treats 401 as a typed `Error` instead of an "unexpected" exception.
...
- 2026-06-01 third Copilot review folded — five findings (one M counted twice on the same file, plus three Ls).
  - **M1 + M2 — `priceType` emitted as `number` on writes, `string` on reads.** Real wire-contract bug. `MVC`'s `JsonStringEnumConverter` (registered in `MakablesControllersExtensions`) emits enums as their **names** at runtime, but `Microsoft.AspNetCore.OpenApi` builds schemas from the type model and doesn't see the converter — so it emitted `{ "type": "integer" }` for every C# enum. Reads were `string` only because their projections call `.ToString()`. The generated `CreateProductRequest.priceType` / `UpdateProductRequest.priceType` typed as `number`; the runtime accepts both forms (so it "worked"), but callers writing `priceType: 0` would silently break on a reordered enum. Added a platform-wide schema transformer at `backend/src/Makables.Config/Extensions/AddMakablesOpenApi.cs` that rewrites every `IsEnum` schema to `{ "type": "string", "enum": [<names>] }`. Replaced all four `builder.Services.AddOpenApi("v1")` calls with `builder.Services.AddMakablesOpenApi("v1")` so Customer / Maker / Admin / Public hosts all emit honest enum schemas. Regenerated the Maker client; `priceType` now types as `PriceType` (a string-valued enum: `Fixed = "Fixed"` etc.). The next Public / Customer regen will pick up the same fix automatically.
  - **L1 — controller comment wrong arity.** The "No 400" comment said "four `[FromBody]` mutations (Create / Update / Delete / RemoveImage)"; only Create + Update take a JSON body. Rewrote: two `[FromBody]` mutations + the multipart upload trip framework-level `ValidationProblemDetails`; Delete / RemoveImage have only path params (the failure surface is just the 401 / 404 declared inline).
  - **L2 — test header sort field.** `GetMyProducts_sorts_active_first_then_newest_created`'s inline comment said "newest first by CreatedAt" — the projection orders by `Id desc` (ULID time-proxy). Updated.
  - **L3 — status-log hash drift.** Each Copilot round triggers a regen → the spec hash in `.spec-hashes.json` changes → the inline hash quoted in this log goes stale. Removed the quoted hashes from this log and pointed readers at the `.spec-hashes.json` file as the source of truth. Future Copilot rounds won't compound the drift.
