...
- 2026-06-02 done. Build clean, 857 tests pass (773 unit + 84 integration; +2 new). Frontend tsc + lint clean. Code-quality review CLEAR (no BLOCKERs, no Mediums; all 6 ACs pinned).
- 2026-06-02 Copilot review folded — 2 findings.
  - **M1 — `FileParameter.fileName` regressed to required.** T-0049b's appendix injected `fileName?: string` (optional) to match the NSwag template's runtime fallback (`file.fileName ? file.fileName : "file"`). T-0049c's regen produced a spec where NSwag now emits `FileParameter` itself with `fileName: string` (required), so the T-0049b appendix's `!declared` guard correctly didn't fire — and the regenerated client reverted to the broken "required" shape. Added a third post-process branch in `generate-api.mjs`: detect NSwag's own declaration and normalise `fileName` to optional. Two branches now: (a) NSwag emits → relax to optional; (b) NSwag doesn't emit → inject the canonical shape (the original T-0049b appendix path). Both write `export interface FileParameter { data: any; fileName?: string; }`. Re-regenerated; verified.
  - **L — `BuildFactory` duplicated.** `MultipartSchemaTransformerTests` shipped its own 50-line `WebApplicationFactory` setup, drift-prone if a future `ValidateOnStart` block lands. Extracted to a shared `HostStartupHarness` (internal static helper, generic `Build<TProgram>()`). `WebHostStartupTests`'s `HostStartupTestBase<T>` delegates to it; `MultipartSchemaTransformerTests` calls it directly. Chose a static helper over a base-class inheritance because inheriting `HostStartupTestBase` would have re-run all 9 `[Fact]` startup-smoke tests against the multipart test class — wasteful.
