...
- **State-machine methods** on `Order` — each returns `BusinessResult` (non-generic for transitions that produce no new value); illegal transitions return `BusinessResult.Failure(Error.Conflict("state", BusinessErrorMessage.OrderInvalidTransition))`:
  - `MarkAsPaid(IClock clock, string paymentProviderRef)` — `PendingPayment → Paid`. Sets `PaidAt` and `PaymentProviderRef` (set-once invariant: refuse with `Error.Conflict("paymentProviderRef", OrderInvalidTransition)` if already set, regardless of current state).
  - `Accept(IClock clock)` — `Paid → Accepted`. Sets `AcceptedAt`.
  - `Ship(IClock clock, string? shippingCarrierRef, int autoDeliverWindowDays = 7)` — `Accepted → Shipped`. Sets `ShippedAt`, `AutoDeliverAt = ShippedAt + window`, `ShippingCarrierRef` when supplied (set-once invariant: refuse if already set; the parameter is nullable because the personal-pickup path in T-0073 has no carrier ref).
...
- **`IOrderRepository`** at `backend/src/Makables.Core.Domain/Orders/IOrderRepository.cs`. Surface per ADR 0013 §"Country and ownership scoping":
  ...
  - `Task<Order?> GetByIdUnscopedAsync(string orderId, CancellationToken ct)` — admin lookups + soft-delete reconciliation.
...
- **Integration** at `backend/src/Makables.Tests/Infra/Orders/OrderRepositoryTests.cs` (against the established `TestDbHarness` SQLite — not `Makables.IntegrationTests`, which is reserved for Testcontainers + `WebApplicationFactory` per the T-0049a precedent):
  ...
  - Migration applies cleanly (the harness builds the schema from the model + applies all migrations on each test).
...
- **AC-10** Given the migration runs on a clean DB, when `dotnet ef database update` is invoked, then it applies cleanly and creates `orders` + all five named indexes. The SQLite test harness applies the migration without error on every test run.
...
### `AutoDeliverAt` window — hard-coded 7 days, default parameter

The role doc says 7 days. ADR 0017 (Packeta) confirms 7 days for `AutoDeliverAt`. Country-driven (read from `CountryConfiguration`) is the architecturally consistent move — multi-country ready. But it's a single-launch market and per-country variation is unlikely. **Decision:** hard-code 7 as the default parameter on `Ship(...)`. The caller (T-0072 `ShipOrder.Handler`) can pass a different window if a future `CountryConfiguration` field is added. The entity stays simple; the policy lives at the command layer. This is one of the open questions; the proposed default below is to surface the trade-off in T-0072.
