...
- Profile header card: company name, verified badge if `IsVerified`, city, rating (stars rendered from `RatingAverageBp ÷ RATING_BP_PER_STAR` where one star = 10 000 bp per `CatalogQueries.BpPerStar`, 1-decimal display) + `RatingCount` + `TotalOrders`, bio paragraph, legal form, personal-pickup pill if `PersonalPickupEnabled`.
...
- Rating: `RatingAverageBp` is basis-points at 10 000 bp per star (`CatalogQueries.BpPerStar`); range 0..50 000. Display as `(RatingAverageBp / RATING_BP_PER_STAR).toFixed(1)` where `RATING_BP_PER_STAR` is exported from `lib/api-client-helpers/catalog.ts` so every catalog surface shares one constant. Render star glyphs purely from the display value — no business logic, just a presentational `<Stars value={x} />` Server Component.
...
- 2026-05-30 Copilot review folded.
  - **Stars rounding mismatch (Low).** The docstring promised "floor of value is filled" but the code used `Math.round`, so 4.5 rendered as five filled stars. Switched to `Math.floor` so the glyph row matches the convention (whole stars to the left of the value) and matches the docstring. 4.4 and 4.9 now both render as four filled stars; the fractional precision lives in the 1-decimal numeric display next to the row (AC-1). Tightened the docstring to call out the example explicitly.
  - **Non-CZK product would 500 the profile route (Medium).** `ProductPrice` was calling `assertCzkCurrency`, which throws — a single contract-violating row (data backfill, future EUR pilot) would crash render and surface as the route's error UI, taking the whole profile down with it. Replaced the assertion with an inline `priceCurrency !== 'CZK'` guard that falls back to the "Na poptávku" copy ("ask the maker" is the right user-facing meaning when we can't display a price). The formatter still asserts on direct calls so dev/CI surfaces drift loudly; only the card boundary became forgiving.
- 2026-05-31 second Copilot review folded.
  - **Nested `<main>` landmarks (Medium ×3).** `app/layout.tsx:26` already wraps `{children}` in `<main>` and T-0046's `/katalog/page.tsx` uses `<section>` as the outermost wrapper for that reason. T-0047's `page.tsx`, `loading.tsx`, and `not-found.tsx` each opened their own `<main>` inside that root `<main>` — two landmarks of role `main` is invalid HTML and an a11y violation. Switched all four occurrences across the three files to `<section>`, matching the T-0046 convention.
  - **Formatter assertion comment was a lie (Medium).** Previous round dropped `assertCzkCurrency` from the call site to keep the route alive on non-CZK input, but the comment promised the formatter still asserts on direct calls — and `formatCzk` never actually invoked `assertCzkCurrency`. Made the comment true: `formatCzk` now takes `(amountMinor, currency)` and calls `assertCzkCurrency(currency)` first thing. The only callsite (`ProductPrice`) now passes `item.priceCurrency`; the upstream `priceCurrency !== 'CZK'` guard still routes non-CZK away from the formatter for the user-facing fallback, so the route stays forgiving while the formatter stays loud. Updated the comment to match. T-0048's `formatCzk` calls will be forced to pass the currency too — no further drift possible.
- 2026-05-31 third Copilot review folded (also fixes T-0046's identical bug).
  - **Rating divisor off by 10× (High ×2).** Backend stores `RatingAverageBp` at 10 000 bp per star (`CatalogQueries.BpPerStar`; the test fixture `SetCatalogStats(45000, ...)` corresponds to 4.5 stars). All three call sites (`page.tsx:73`, `page.tsx:126`, and the carried-over `maker-card.tsx:20` from T-0046) divided by 1 000 — so a 4.5-star maker rendered as "45.0" next to a 5-star row clamped by `Math.min(5, …)`. Every maker with any rating displayed as five filled stars. Added a `RATING_BP_PER_STAR = 10_000` constant to `lib/api-client-helpers/catalog.ts` (single source of truth, mirrors the backend constant) and updated all three sites to divide by it. T-0046's and T-0047's ticket docs claiming `÷1000` were also out of step; corrected those to reference the new constant + the backend `BpPerStar`.
- 2026-05-31 fourth Copilot review folded.
  - **Czech plural-genitive bug (Medium).** `'{count} dokončených objednávek'` and `'{rating} ({count} hodnocení)'` use the Czech genitive-plural form, which is only correct for 0 and 5+. For count = 1 Czech needs singular ("1 dokončená objednávka") and for 2-4 the nominative plural ("2 dokončené objednávky"); a maker with 1 completed order would render "1 dokončených objednávek". Same trap on T-0046's `'catalog.card.orders'` (`'{count} objednávek'`) and `'catalog.pagination.results'` (`'{count} výrobců'`) — Copilot only flagged T-0047 but the carry-over deserves the same fix. The proper solution is `Intl.PluralRules('cs')` in `t()`; that's a runtime change worth a follow-up ticket. For now switched to grammatically-neutral phrasing that's correct for every count: `'Dokončených objednávek: {count}'`, `'{rating} (hodnocení: {count})'`, `'Objednávek: {count}'`, `'Výrobců: {count}'`. Added an inline comment in `cs-CZ.ts` explaining the convention so T-0048's strings follow suit.
  - **Misleading i18n namespace (Low).** `'catalog.maker.not_found.back_to_catalog'` was used by both the not-found page and the happy-path profile footer; reading the page code, that key path looks wrong. Renamed to `'catalog.maker.back_to_catalog'` (one level up, neutral) and updated both call sites.
