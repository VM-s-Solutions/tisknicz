...
- 2026-05-26 reviewer fixes folded into one commit. 670 tests still pass.
  - **Security B1** (CORS-with-credentials dev fallback ships unconditionally) — **closed**. `AddMakablesCors` now requires an explicit `IHostEnvironment` and fails-closed with an `InvalidOperationException` if `Cors:AllowedOrigins:{audience}` is empty outside the Development / IntegrationTest environments. All four host `Program.cs` files updated. Integration test config files supply explicit allowed-origins per audience (documentation of prod-shape).
  - **Code-quality M-1** (none) / **Code-quality MAJORs** (none — only minors).
  - **Code-quality m1 + m2** (semantic key reuse in confirmation banner + magic-link success copy) — closed. Added `auth.verify.banner_sent` and `auth.magic.request_done_body` keys; updated the two call sites.
  - **Code-quality m3** (UserAgent empty-string passes through to handler) — closed. Added `AuthController.NormalizedUserAgent()` helper that returns null when the header is absent.
  - **Code-quality m4** (Logout result discarded silently) — closed. Wrapped the Mediator call in `try / finally`, used `_ = await` to document the discard, and moved `ClearSessionCookies` into the `finally` so an infra exception can't leave the user half-logged-in (also closes sec m4).
  - **Code-quality m5** (apiFetch spread ordering — `credentials` could be overridden by `...options`) — closed. Reordered so the spread runs first; the explicit `credentials: options.credentials ?? 'include'` now wins, and a caller passing `credentials: 'omit'` is still honoured.
  - **Security M1 + m1 + m3 + n2** — documented as known follow-ups: anonymous-endpoint rate-limiting (per-IP / per-email partitions for login / request-password-reset / request-magic-link); CSRF posture doc for unauthenticated POSTs; refresh-cookie SameSite=Strict load-bearing comment; useEffect StrictMode double-fire on `/auth/verify` and `/auth/magic` (token re-use is rejected by the second call — already safe but worth a comment in a polish ticket).
  - **NITs** (`auth.ts:35` `userId` unread, login-form TODO link, Spinner aria-label, AuthCookies Domain comment) — deferred to a polish ticket.
