...
- 2026-06-05 second Copilot review pass on PR — surfaced 4 findings (1 Medium + 3 Lows) on the email-confirmed gate threat-model comments and OrdersController 401 XML doc. Workflow-verified (12/12 lenses real). I claimed in an audit-trail commit (`bf08752`) that all four were already resolved in the rebased state. **That claim was wrong** — I misread the file-content output during spot-checks (likely conflated the workflow agent's quoted recommendations with the actual file content). The R3 pass below catches the actual mistake.
- 2026-06-05 third Copilot review pass on PR — surfaced 5 findings (3 Mediums + 2 Lows) on the same surfaces. Honest spot-check this time: **4 of 5 findings were genuine code/doc defects** that the R1 H-1/M-2/L-1/L-2 commit (`655a941`) did NOT touch; my R2 audit was incorrect. The R3 fold actually applies them:
  - **R3-1 (FOLDED):** `JwtIssuer.cs:78-80` claimed "a forged claim of `0` on a future evil token still fails the middleware's null check" — wrong: middleware's `!string.IsNullOrEmpty` would PASS `"0"`. Rewrote to credit JWT signature validation as the actual safety mechanism: "Add email_confirmed_at only when set so unconfirmed users structurally omit the claim (T-0063). The gate relies on JWT signature validation: callers cannot forge this claim without the server signing key."
  - **R3-2 (FOLDED):** `MakablesClaimTypes.cs:22-23` had the same "fail closed" framing. Rewrote to: "JWT signature validation prevents clients from forging this claim; unconfirmed users simply omit it from the token."
  - **R3-3 (already-correct):** middleware XML doc `Bypasses` paragraph. Verified by `git log` it was already trimmed in the original feature commit `7a07b16` — no health/openapi false claim ever shipped. **R3-3 is the only genuine R2/R3 phantom.**
  - **R3-4 (FOLDED):** middleware **inline** comment at `RequireEmailConfirmedMiddleware.cs:80-83` (NOT the XML doc; the comment immediately above the `FindFirstValue` call) carried the same `"Absence (not '= 0')" is the unconfirmed signal — see MakablesClaimTypes.EmailConfirmedAt remarks on why we don't emit zero for unconfirmed users` wording. Rewrote to: "Confirmed if the claim is present at all; unconfirmed users structurally omit it. The gate relies on JWT signature validation: callers cannot add this claim to a token without the server signing key (see MakablesClaimTypes.EmailConfirmedAt remarks for the wider rationale)."
  - **R3-5 (FOLDED):** `OrdersController.cs:70-76` XML doc said "401 is intentionally not declared via [ProducesResponseType] because..." but the attribute IS declared below. Rewrote to: "401 is declared via [ProducesResponseTypeAttribute] for OpenAPI completeness even though, on the normal controller path, the framework's authentication challenge can short-circuit before the handler runs. The handler still has an Unauthorized backstop for non-controller callers (e.g. a future cron), which returns a typed Error."
  - All 4 fixes are comment/XML-doc edits — no behavioural code change. Build clean. 994 tests still pass (899 unit + 95 integration). The two integration tests covering the affected middleware branch (`CreateOrder_returns_403_AuthEmailNotConfirmed_when_user_email_unconfirmed` and `RequireEmailConfirmedMiddleware_bypasses_auth_subpath_for_authenticated_unconfirmed_user`) both pass post-edit.
