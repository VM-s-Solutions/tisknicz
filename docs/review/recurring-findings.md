...
| 2 | Ticket/code ships a new `BusinessErrorMessage` constant without its parallel `cs-CZ` i18n key — caught only at Gate 9 contract/i18n parity, never at authoring time | 3 | order-cleanup bundle, order-dashboards bundle, payout-core bundle (`csvPathAlreadySet`) | "Every new `BusinessErrorMessage` constant MUST ship with a matching `cs-CZ` i18n key in the same PR. Enforce mechanically: a pre-commit or CI check asserting each `BusinessErrorMessage` code has a parallel `cs-CZ` key (Architect/secops to design). **Standing automated-gate candidate** — third strike fired at payout-core; this is no longer a checklist line, it wants a script." | codified-in-script — [`ruleT8`](../../scripts/check-consistency.mjs) (T-0125, allowlist-driven, `hard:true`; allowlisted codes use the `errors.ts` type-fallback) |
| 3 | A new partial/unique index ships in a migration + EF config but is NOT registered in `UniqueConstraintTranslator.Mappings` — so a concurrent write hitting the 23505 unique-violation falls through to a raw HTTP 500 instead of the intended `BusinessResult` conflict (`ReviewAlreadyExists`, `PayoutBatchAlreadyOpen`, …). App-level `Exists*Async` pre-check masks it until a true concurrent race | 2 | payout-core bundle (`ux_payout_batches_open_per_country`, `ux_payout_batches_country_batch_number`), reviews-loop bundle (`ux_reviews_order_active`) | "Every unique/partial-unique index that backs a user-facing uniqueness rule MUST have a matching `UniqueConstraintTranslator.Mappings` entry mapping its 23505 to the intended `BusinessErrorMessage`, AND a concurrent-write integration test asserting the race loser gets the conflict (not a 500). Enforce mechanically: a check pairing every `IsUnique()`/`CREATE UNIQUE INDEX` name against the translator's registered keys (Architect to design — directory/AST walkable). One strike from codification." | codified-in-script — [`ruleT9`](../../scripts/check-consistency.mjs) (T-0125, NAMED unique indexes only, `hard:true`; satisfy via translator key OR a `// no-translator:` marker) |
