...
| [custom-domain-and-tls.md](./custom-domain-and-tls.md) | Binding `makables.cz` and `admin.makables.cz` to the production frontend and issuing the free managed certificates. Runs AFTER the app is provisioned (both DNS records are derived from the App Service) and BEFORE any user-facing email. Also re-registers the OAuth redirect URIs, which the cutover silently breaks. Only the frontend App Service needs a domain — the API hosts are reached through the same-origin `/api-proxy` rewrite. |
