...
- **Update 2026-08-31:** `UseMakablesForwardedHeaders` has now landed (first stage of `UseMakablesPipeline`, enabled in deployed environments, `ForwardLimit = 1`). This does **not** close this question. It restores the real client IP only for callers that reach a host *directly* — the Comgate webhook, direct API clients — which is what it was built for. Browser traffic still arrives through the Next `/api-proxy` rewrite, so the last forwarded hop remains the frontend App Service egress IP and the shared-bucket problem is unchanged. Option 2 above is therefore still not satisfied by the middleware alone: as that option already notes, `rewrites()` cannot inject the client address, so it would additionally need the proxy to become a Route Handler that forwards it. Option 1 (retire the proxy behind a shared parent domain) remains the clean fix.
- **Status:** open
- **Answer (filled by user):**
