...
| P1 | Prod Postgres network path | L | ✅ **done** — shipped in `infra/bicep/modules/network.bicep` (VNet + private endpoint + private DNS zone + vnet link, prod-gated on `enablePrivateNetworking`). Two operator checks remain after the first prod deploy because ARM reports success either way. Decision as taken: Private endpoint (not VNet injection: that is creation-time-only and `publicNetworkAccess` cannot be combined with it). PE coexists with public access + firewall rules, which is what keeps the CI temp-firewall migrate job working |
| P2 | Custom domain + DNS + TLS binding | M | ⏳ **runbook written, binding NOT performed** — [custom-domain-and-tls.md](../runbooks/custom-domain-and-tls.md). **Correction: this runs AFTER the first prod deploy, not before.** The earlier note here said the opposite. Both DNS records are derived from the App Service (the apex `A` needs its inbound IP, the `asuid` TXT its verification ID), so provisioning must come first; and `PublicAppUrlsOptionsValidator` only checks that the URL is well-formed https — it never resolves the host, so the hosts boot fine against an unbound domain. What the domain actually gates is the go-live data chain, because the first maker registration sends unrecallable email carrying `https://makables.cz` links. Only the frontend App Service needs a domain (two hostnames); the API hosts are reached through the same-origin `/api-proxy` rewrite |
| P3 | First-admin bootstrap | M | ✅ **done** — `Makables.Tools.AdminBootstrap`, an operator-run CLI: password from stdin never argv, 12-char floor, `--confirm-database` must match the resolved DB, refuses when an active admin already exists, writes an audit row. `Register.cs` rejects `UserRole.Admin`; the seeder hard-refuses any target containing `prod` |
| P4 | Gate the public catalog on `Maker.IsVerified` | S | ✅ **done** — `m.IsVerified` now gates every public read in `CatalogQueries.cs`. Becomes load-bearing in prod, which has no pre-verified seeded makers |
| P5 | Go-live data runbook | M | ✅ **done** — [go-live-bootstrap.md](../runbooks/go-live-bootstrap.md). A fresh prod DB has reference data but zero makers/products. Chain: bootstrap admin → maker registers → ARES → admin verifies → maker creates product. Contains an external party; cannot be compressed into a deploy window |
| P7 | Prod blob durability | S | ✅ **done** — `Standard_GZRS` in prod (dev stays LRS) plus 30-day blob **and container** soft delete in every env. ADR 0023 §7 amended GRS → GZRS: under GRS the primary-region copy is LRS, so a single West Europe datacenter loss needs a lossy manual failover, and the zone axis is not a live SKU update — the first-ever deploy was the only free moment. Versioning deliberately excluded: it silently breaks `az storage blob undelete` |
