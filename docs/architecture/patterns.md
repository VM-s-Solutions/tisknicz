...
**Shipped:** Google OAuth (T-0026/T-0035, ADR 0012). **Planned/in progress:** Apple Sign-In (T-0139, ADR 0026). **Out of scope for MVP:** MFA, SSO. Wrapped behind `IAuthService` / the OAuth-provider seam below so adding further providers doesn't ripple through the codebase.

**A.17.1 OAuth provider pattern (server-side authorization-code flow)** — reusable reference for *any* provider (Google, Apple, and future providers), not provider-specific:

- **State signing.** `IOAuthStateSigner` (`Core.Domain.Identity`, impl `Infra.Common.Auth.OAuthStateSigner`) mints an HMAC-signed state binding `Audience`, `RedirectUri`, `CsrfCookieHash` (SHA-256 of an HttpOnly anti-CSRF cookie value), `Nonce`, and `IssuedAt`. The HMAC key is HKDF-derived from the JWT signing key under a dedicated context label (`makables-oauth-state-v1`) so a JWT can never be mistaken for OAuth state or vice versa. `TryVerify` requires an **exact match** of `RedirectUri` and the CSRF-cookie hash presented at the callback, plus a stale-window check (10 minutes). Every new provider's `Start<Provider>OAuth` / `Complete<Provider>OAuth` pair reuses this signer unchanged — do not add a per-provider signing key.
  - The signed payload carries **no explicit `Provider` claim**. Provider discrimination is structural, not a signed field: each provider is given its own, distinct callback route (`/auth/google/callback`, `/auth/apple/callback`, ...), and `RedirectUri` is exact-match-bound into the state — a state minted for one provider's redirect URI cannot pass another provider's callback verification. This is reinforced by the fact that the OAuth `code` itself is provider-specific and non-transferable between issuers' token endpoints. See ADR 0026's Defense section for the full analysis. **Rule for implementers:** never configure two providers to share the same redirect URI — that is the one way this implicit discrimination breaks.
- **Admin-audience rejection.** Every OAuth provider is rejected for `audience=admin` at both the start and complete handlers (defense-in-depth: checked twice). Admins authenticate with password + (future) MFA only — this is a standing rule from ADR 0012, applied identically to every provider, not something each new provider's ADR re-decides.
- **Account merge by email.** `EmailNormalized` is the convergence key across every identity path (password, magic link, every OAuth provider). First OAuth login for an existing account (password or another provider) links that provider's subject id (`GoogleSub`, `AppleSub`, ...) to the existing `User` row and confirms the email (the provider has already verified it) — never creates a duplicate account. New accounts get `Role` from the signed `Audience` and `CountryCode` from `AuthDefaultCountryOptions`.
- **Provider-specific deltas stay isolated to `Infra.Clients/<Provider>/`.** Client-secret mechanics (static vs signed JWT), callback HTTP shape (GET query string vs POST form body), and profile-delivery quirks (always-present vs first-authorization-only fields) are the only things that vary per provider — documented per-provider in that provider's ADR (e.g. ADR 0026 for Apple), not repeated here.
