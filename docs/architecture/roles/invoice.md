...
- CountryConfiguration columns: 4 new (`issuer_name VARCHAR(200) NOT NULL`, `issuer_ico CHAR(8) NOT NULL`, `issuer_dic VARCHAR(15) NULL`, `platform_iban VARCHAR(34) NULL`). Migration `20260607104759_AddCountryConfigurationIssuerAndIban.cs`. CZ seed: `issuer_name='JVM Yore, s.r.o.'`, `issuer_ico='29633443'`, `issuer_address='Příčná 1892/4, Nové Město, 110 00 Praha 1'` (the ARES record, verified 2026-08-23 — the `'00000000'` placeholder and its `country-config-ico-replace-placeholder-pre-launch` manual_step are closed), `issuer_dic=NULL` (not VAT-registered), `platform_iban=NULL`. `issuer_address VARCHAR(200) NULL` was added by migration `20260823205941_InvoiceIssuerAddressAndSettlement`. Renderer skips SPAYD QR rendering when `platform_iban IS NULL`.
...
**T-0194b shipped (document redesign + settlement snapshot):**

- Templates extracted out of the renderer: `backend/src/Makables.Infra.PdfRendering/InvoiceDocument.cs` holds `InvoiceDocumentBase` (masthead, party blocks, metadata band, totals, settlement stamp, footer — drawn once) plus the three concrete documents `DokladOProdejiDocument` / `DanovyDokladDocument` / `ProvizniDokladDocument`, which now differ only in title, recipient label and line-item table. `QuestPdfInvoiceRenderer.cs` is reduced to the mode switch.
- Design tokens: `backend/src/Makables.Infra.PdfRendering/InvoiceTheme.cs` transcribes the LIGHT half of `frontend/src/app/globals.css` (paper is white, so the light ramp is the correct one) plus the Makables mark as inline SVG re-inked to `--lt-brand-400` — the web asset's `#2dd4bf` is a 1.9:1 wash on white. `InvoiceFormatting` moved here too and is unit-tested via `InternalsVisibleTo(Makables.Tests)`.
- Settlement snapshot: `invoices.paid_on DATE NULL` + `invoices.payment_method VARCHAR(60) NULL`, migration `20260823205941_InvoiceIssuerAddressAndSettlement`. `Invoice.Issue` rejects a payment method without a settlement date. `IssueInvoice.Handler` snapshots `Order.PaidAt` (country-local) + `Order.PaymentMethod`; `PayoutArtifactService` snapshots the batch DUZP + `SettlementMethods.PayoutDeduction`, because the payout is already net of `Order.PlatformFeeAmountMinor`.
- **The document is a receipt, not a payment request.** `PaidOn` drives the whole lower half: totals read "Celkem" not "Celkem k úhradě", an UHRAZENO stamp carries date + channel, and the due date / variable symbol / SPAYD block are suppressed entirely. Printing "Celkem k úhradě 739 Kč" at a customer who had already paid was the reported defect. Both families the platform issues today are settled before their document exists, so the outstanding branch is unreachable in production — it is kept because it is the honest else-branch of a nullable column, and it is pinned by tests.
- Order reference: `invoices.order_number VARCHAR(40) NULL`, migration `20260823212428_InvoiceOrderNumberSnapshot` (backfilled from `orders.order_number` over the `order_id` FK). The templates previously printed the invoice number's own numeric tail as "Objednávka {tail}", which matches no order the customer can look up; rows predating the column fall back to that tail.
- Issuer seat: `country_configuration.issuer_address` + `invoices.issuer_address`, snapshotted at issuance so a relocation never rewrites a historical document. Required on a § 29 daňový doklad; the documents previously carried no address at all.
